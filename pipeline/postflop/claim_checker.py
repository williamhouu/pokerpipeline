...
POSTFLOP_CHECKER_SYSTEM_PROMPT = (
    ...
    "  * A size-character claim that contradicts the SIZE COMPOSITION lines. "
    "When present, those lines resolve what each bet size's betting range "
    "looks like (value-heavy / merged / polarized / bluff-heavy) and the SIZE "
    "COMPARISON line resolves which size is the more polarized one. Flag prose "
    "that reverses a stated character (calls a size polarized when its line "
    "says merged or value-heavy) or picks the wrong size as the polarized/"
    "merged one. When there are NO SIZE COMPOSITION lines, general sizing "
    "principles are fine -- do not flag them.\n"
