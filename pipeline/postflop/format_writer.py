...
# (July 2026, team request) -- the same declutter the NLHE preflop writer did
# in June 2026 (_PREFLOP_DROPPED_COLUMNS): the dropped values are flat
# duplicates of numbers already inside the kept ``stat_notes`` JSON (which the
# admin math panel falls back to) or difficulty diagnostics. ``sequence_total``
# is also dropped (team, July 2026): the app groups on ``hand_id`` and orders
# by ``sequence_index``, and a leg count is just the group size. ``hand_id`` /
# ``sequence_index`` and ``hand_difficulty`` (the app's hand-level selector)
# are KEPT. Standalone postflop batches keep the full ``POSTFLOP_CSV_COLUMNS``
# schema unchanged.
...
FULL_HAND_CSV_COLUMNS: tuple[str, ...] = tuple(
    c for c in POSTFLOP_CSV_COLUMNS if c not in _FULL_HAND_DROPPED_COLUMNS
)
