...
def build_preflop_entry_row(
    ...
    *,
    ...
    return {
        ...
        "easy_concept": "",
        "easy_hand": "",
        # Notes carries provenance + chart + situation + node reference (the
        # old solver_reference value, now the Node: field). July 2026.
        "Notes": build_notes(
            "Auto-generated by poker-pipeline (preflop-entry from postflop solve).",
            chart=f.solve.source_reference,
            situation=f"{f.hero_position} vs {f.villain_position}, {f.archetype}",
            node_ref=f"{f.solve.source_reference}/preflop/{f.hero_position}/{f.hero_combo}",
        ),
