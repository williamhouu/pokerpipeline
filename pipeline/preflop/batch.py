...
# Near-pure threshold: a dominant action at/above this frequency is an
# essentially-decided (EASY) spot. The tiny mixed action is GTO balancing at
# ~equal EV, so the spot's EV gap is ~0 -- which would mis-rate it HARD on the
# difficulty EV axis. We hand the EV axis full credit at/above this frequency so
# near-pure spots read easy. This is the ONLY thing the old snap-to-pure feature
# still does: the display rounding (96-99% shown as 100%, with the CSV freqs and
# per-action EVs blanked) was dropped June 2026 -- the CSV + SOLVER DATA now
    ...
    *,
    ...
    for spot, _evaluation in worthy:
        ...
        try:
            ...
            # Near-pure spots (>= 96% dominant) are EASY decisions, but the tiny
            # mixed action is GTO balancing at ~equal EV, so the spot's EV gap is
            # ~0 -- which would mis-rate it HARD on the difficulty EV axis. Give
            # the EV axis full credit for DIFFICULTY only; the CSV + SOLVER DATA
            # still carry the real frequencies + per-action EVs (no display snap).
