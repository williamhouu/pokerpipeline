...
def _format_real_mix(freqs: dict[str, float]) -> str:
    """A snapped spot's true solver mix as ``"Fold 99%, Call 0%"`` (descending)."""
    return ", ".join(
...
def _notes_column(snapped_real_freqs: dict[str, float] | None) -> str:
    """The Notes provenance string, plus the snap note for a snapped-to-pure spot.

    When a near-pure spot is displayed as 100% (the question + CSV round it for
    teaching), record the solver's REAL mix here so the raw CSV is self-explaining
    -- the panel shows the same truth in its "Solver frequencies" line.
    """
    notes = "Auto-generated by poker-pipeline (preflop path)."
    if snapped_real_freqs:
        notes += (
            " The question and CSV round to 100% for teaching; the solver's "
            f"real mix is {_format_real_mix(snapped_real_freqs)}."
        )
    return notes
...
def build_preflop_row(
    ...
    *,
    ...
    return {
        ...
        "Notes": _notes_column(snapped_real_freqs),
        # Pipeline columns.
...
def write_preflop_csv(
    ...
    *,
    ...
    snapped_freqs: Iterable[dict[str, float] | None] | None = None,
) -> int:
    ...
    Args:
        ...
        row_statuses: Optional per-row ``validation_status`` overrides,
            ...
            warned on.
        snapped_freqs: Optional per-row real solver mix (positionally aligned
            with ``rows``) for spots snapped-to-pure; when present, the row's
            Notes column records the true mix behind the rounded-to-100%
            display.
    ...
    snap_list = list(snapped_freqs) if snapped_freqs is not None else []
    written = 0
    ...
    with open(out_path, "w", newline="", encoding="utf-8-sig") as handle:
        ...
        for number, (facts, explanation, difficulty) in enumerate(rows, start=1):
            ...
            row: dict[str, Any] = build_preflop_row(
                ...
                snapped_real_freqs=(
                    snap_list[number - 1]
                    if number - 1 < len(snap_list) else None
                ),
            )
            writer.writerow(row)
            written += 1
