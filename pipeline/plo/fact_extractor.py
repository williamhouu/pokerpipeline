...
from pipeline.plo.hand_order import combo_multiplicity
...
# --- main entry point -------------------------------------------------------
def extract_plo_facts(spot: PloSpot, pack: PloPack) -> PloFacts:
    """Compute the per-spot structural facts Layer 6 needs.

    Always returns a valid :class:`PloFacts` -- if villain range loading fails,
    the villain fields stay ``None`` rather than raising, so one malformed spot
    can't bring down a batch.
    """
