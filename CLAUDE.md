...
Source of truth is **Google Sheets**. The schema is **44 columns**. The
...
| 41 | `easy_hand` | Per-spot ease on the hand-class axis. U-shaped: premium hands AND clear trash (incl. suited junk like 73s) are easy; marginal hands are hard. Preflop only. |
| 42 | `difficulty_bumps` | Comma-separated names of any `BUMP_RULES` from `pipeline/preflop/difficulty.py` that fired for this spot. Empty when no bumps fired (current default: the table is empty pending observed mis-scored patterns). |
| 43 | `hand_class` | single label, e.g. `top_pair_with_flush_draw` (preflop: the 169-class label like `AKo`) |
| 44 | `Notes` | provenance string, e.g. "Auto-generated by poker-pipeline (preflop path)." |
...
- **Sheet 1 "Example formatting"** — 6 fully-populated example rows. The baseline
  ...
  diagnostics, May 2026), `Position Matchup` (May 2026 reorg), and `ranges`
  (multiway-capable position-labeled range JSON, May 2026). The three empty
  `tag_1`/`tag_2`/`tag_3` placeholder columns were dropped in the reorg —
  current total 44 columns.
