...
| 33 | `archetype` | preflop strategic frame: one of the 19 labels in `pipeline.preflop.explanation_generator.PREFLOP_ARCHETYPE_GUIDANCE` (`open_for_value`, `3bet_for_value`, `squeeze_as_bluff`, `fold_dominated`, `call_allin` — a pure pot-odds call of a jam, NO implied odds —, `sb_complete` — SB first-in limp, ported from PLO for the 9-max pack (June 2026) — etc.) or `unclassified`. Empty for postflop rows (postflop has no archetype layer). The LLM already gets this in its SOLVER DATA block as the strategic frame; the CSV column is for analytics + reviewer QA. (May 2026.) |
