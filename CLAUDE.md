...
| 28 | `Notes` | provenance string, e.g. "Auto-generated by poker-pipeline (preflop path)." **Moved here — right before `ev_gap_bb` — June 2026.** |
| 29 | `ev_gap_bb` | EV gap to second-best action; `<0.30` surfaces questionable questions |
...
| 33 | `archetype` | preflop strategic frame: one of the 16 labels from `pipeline.preflop.fact_extractor.classify_archetype` (`open_for_value`, `3bet_for_value`, `squeeze_as_bluff`, `fold_dominated`, etc.) or `unclassified`. Empty for postflop rows (postflop has no archetype layer). The LLM already gets this in its SOLVER DATA block as the strategic frame; the CSV column is for analytics + reviewer QA. (May 2026.) |
...
- **Sheet 1 "Example formatting"** — 6 fully-populated example rows. The baseline
  ...
  Cards`); `Notes` also moved to before `ev_gap_bb` — current total
  40 columns.
